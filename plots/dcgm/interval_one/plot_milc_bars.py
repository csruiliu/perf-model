...
value_cols = [c for c in milc_fp64_a100_ref_df.columns if c != "category"]
milc_fp64_a100_ref_data = np.array(
    [
        milc_fp64_a100_ref_df[milc_fp64_a100_ref_df["category"] == cat][value_cols]
        .mean(axis=0)
        .astype(int)
        .values
...
milc_fp64_h100_ref_df = pd.read_csv("milc-fp64-h100-ref.csv")
categories_csv = list(dict.fromkeys(milc_fp64_h100_ref_df["category"]))
value_cols = [c for c in milc_fp64_h100_ref_df.columns if c != "category"]
milc_fp64_h100_ref_data = np.array(
    [
        milc_fp64_h100_ref_df[milc_fp64_h100_ref_df["category"] == cat][value_cols]
        .mean(axis=0)
        .astype(int)
        .values
        for cat in categories_csv
...
data = np.concatenate((milc_fp64_a100_ref_data, milc_fp64_h100_ref_data), axis=0)
...
fig.tight_layout()
plt.savefig("milc_fp64.png", dpi=150)

########################################################

categories = ["H100-SXM", "A40", "RTX8000"]

milc_fp32_a100_ref_df = pd.read_csv("milc-fp32-a100-ref.csv")
categories_csv = list(dict.fromkeys(milc_fp32_a100_ref_df["category"]))
value_cols = [c for c in milc_fp32_a100_ref_df.columns if c != "category"]
milc_fp64_a100_ref_data = np.array(
    [
        milc_fp32_a100_ref_df[milc_fp32_a100_ref_df["category"] == cat][value_cols]
        .mean(axis=0)
        .astype(int)
        .values
        for cat in categories_csv
...
for i, col in enumerate(columns):
    ...
    ax.bar_label(bars, padding=2, fontsize=13)
...
fig.tight_layout()
plt.savefig("milc_fp32_ref_a100.png", dpi=150)

########################################################

categories = ["A100-40GB", "A40", "RTX8000"]
...
fig.tight_layout()
plt.savefig("milc_fp32_ref_h100.png", dpi=150)
