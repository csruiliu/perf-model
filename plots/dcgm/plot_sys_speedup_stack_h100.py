...
# y-axis mode -> left-axis label. For the normalized modes the unit is carried
# by the tick labels themselves, so the axis label stays short.
YMODE_LABELS = {
    "absolute": "Node-hours",
...
def _nice_upper_limit(value):
    ...
    frac = value / 10**exp
    for step in (1.0, 1.1, 2.0, 2.5, 5.0, 10.0):
        if frac <= step:
            return step * 10**exp
    return 10 ** (exp + 1)
