...
split_configs = {
    ...
    1: {  # 128B ground-truth column
        ...
        'colors': ['#B0E0B0', '#6DBF6D', '#3A9E3A', '#1F7A1F', '#0F5C0F', '#073D07'],  # shades of green
    },
...
bars1 = ax.bar(x - bar_width / 2, measurement_count, bar_width,
               label='Ground-truth', color='steelblue', alpha=1)
...
for i in range(len(x_labels)):
    if i in split_configs:
        ...
        continue
    bar = ax.bar(pred_x[i], prediction_count[i], bar_width,
...
for idx, cfg in split_configs.items():
    bottom = 0
    split_bars = []
    for j, (count, color) in enumerate(zip(cfg['counts'], cfg['colors'])):
        bar = ax.bar(pred_x[idx], count, bar_width,
...
for i, (bar, size) in enumerate(zip(bars1, measurement_size)):
    ax.text(bar.get_x() + bar.get_width() / 2,
            bar.get_height() + 500,
            f'{size}B', ha='center', va='bottom', fontsize=14,
...
for i, bar_container in enumerate(bars2):
    ...
    ax.text(bar.get_x() + bar.get_width() / 2,
            bar.get_height() + 500,
            f'{prediction_size[i]}B', ha='center', va='bottom',
