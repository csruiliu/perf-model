...
split_configs = {
    0: {
        'send': {
            0: {'counts': [13711]*11, 'sizes': [12,14,16,20,24,28,32,40,48,56,64],
                'colors': ['#F3E5F5','#E1BEE7','#CE93D8','#BA68C8','#AB47BC',
                           '#9C27B0','#8E24AA','#7B1FA2','#6A1B9A','#4A148C','#2D0060']},
            1: {'counts': [43655]*5, 'sizes': [224,256,320,384,448],
                'colors': ['#FFD580','#FFA500','#FF8C00','#FF6600','#CC4400']},
        },
        'recv': {
            0: {'counts': [13539]*11, 'sizes': [12,14,16,20,24,28,32,40,48,56,64],
                'colors': ['#F3E5F5','#E1BEE7','#CE93D8','#BA68C8','#AB47BC',
                           '#9C27B0','#8E24AA','#7B1FA2','#6A1B9A','#4A148C','#2D0060']},
            1: {'counts': [43985]*5, 'sizes': [224,256,320,384,448],
                'colors': ['#FFD580','#FFA500','#FF8C00','#FF6600','#CC4400']},
        },
    },
    1: {
        'send': {
            0: {'counts': [13539]*11, 'sizes': [12,14,16,20,24,28,32,40,48,56,64],
                'colors': ['#F3E5F5','#E1BEE7','#CE93D8','#BA68C8','#AB47BC',
                           '#9C27B0','#8E24AA','#7B1FA2','#6A1B9A','#4A148C','#2D0060']},
            1: {'counts': [43727]*5, 'sizes': [224,256,320,384,448],
                'colors': ['#FFD580','#FFA500','#FF8C00','#FF6600','#CC4400']},
        },
        'recv': {
            0: {'counts': [13476]*11, 'sizes': [12,14,16,20,24,28,32,40,48,56,64],
                'colors': ['#F3E5F5','#E1BEE7','#CE93D8','#BA68C8','#AB47BC',
                           '#9C27B0','#8E24AA','#7B1FA2','#6A1B9A','#4A148C','#2D0060']},
            1: {'counts': [43713]*5, 'sizes': [224,256,320,384,448],
                'colors': ['#FFD580','#FFA500','#FF8C00','#FF6600','#CC4400']},
        },
    },
    2: {  # Node 2 — replace with your actual split data
        'send': {
            0: {'counts': [13268]*11, 'sizes': [12,14,16,20,24,28,32,40,48,56,64],
                'colors': ['#F3E5F5','#E1BEE7','#CE93D8','#BA68C8','#AB47BC',
                           '#9C27B0','#8E24AA','#7B1FA2','#6A1B9A','#4A148C','#2D0060']},
            1: {'counts': [43003]*5, 'sizes': [224,256,320,384,448],
                'colors': ['#FFD580','#FFA500','#FF8C00','#FF6600','#CC4400']},
        },
        'recv': {
            0: {'counts': [13578]*11, 'sizes': [12,14,16,20,24,28,32,40,48,56,64],
                'colors': ['#F3E5F5','#E1BEE7','#CE93D8','#BA68C8','#AB47BC',
                           '#9C27B0','#8E24AA','#7B1FA2','#6A1B9A','#4A148C','#2D0060']},
            1: {'counts': [43710]*5, 'sizes': [224,256,320,384,448],
                'colors': ['#FFD580','#FFA500','#FF8C00','#FF6600','#CC4400']},
        },
    },
    3: {  # Node 3 — replace with your actual split data
        'send': {
            0: {'counts': [13525]*11, 'sizes': [12,14,16,20,24,28,32,40,48,56,64],
                'colors': ['#F3E5F5','#E1BEE7','#CE93D8','#BA68C8','#AB47BC',
                           '#9C27B0','#8E24AA','#7B1FA2','#6A1B9A','#4A148C','#2D0060']},
            1: {'counts': [43715]*5, 'sizes': [224,256,320,384,448],
                'colors': ['#FFD580','#FFA500','#FF8C00','#FF6600','#CC4400']},
        },
        'recv': {
            0: {'counts': [13457]*11, 'sizes': [12,14,16,20,24,28,32,40,48,56,64],
                'colors': ['#F3E5F5','#E1BEE7','#CE93D8','#BA68C8','#AB47BC',
                           '#9C27B0','#8E24AA','#7B1FA2','#6A1B9A','#4A148C','#2D0060']},
            1: {'counts': [43696]*5, 'sizes': [224,256,320,384,448],
                'colors': ['#FFD580','#FFA500','#FF8C00','#FF6600','#CC4400']},
        },
    },
}
...
# -------------------------------------------------------------------
def draw_subplot(ax, node_id, measurement_count, prediction_count,
                 pred_size_labels, split_cfg):

    offsets    = {'send_meas': -1.5, 'send_pred': -0.5,
                  'recv_meas':  0.5, 'recv_pred':  1.5}
    colors     = {'send_meas': 'steelblue', 'send_pred': 'coral',
                  'recv_meas': 'mediumseagreen', 'recv_pred': 'gold'}
    ...
    for direction_key, meas_data, offset_key in [
        ('send', measurement_count['send'], 'send_meas'),
        ('recv', measurement_count['recv'], 'recv_meas'),
    ]:
        bars = ax.bar(x + offsets[offset_key] * bar_width, meas_data, bar_width,
                      label=bar_labels[offset_key],
                      color=colors[offset_key],
                      hatch=hatches[offset_key],
                      edgecolor='white',
                      alpha=1)
        for bar, size in zip(bars, measurement_size):
            ax.text(bar.get_x() + bar.get_width() / 2,
                    bar.get_height() + 500,
                    f'{size}B', ha='center', va='bottom', fontsize=11,
                    color=colors[offset_key], fontweight='bold')
    ...
    for direction_key, pred_data, pred_x, offset_key, pred_sizes in [
        ...
        for i in range(len(x_labels)):
            if i in cfg:
                bottom = 0
                split_bars = []
                for j, (count, color) in enumerate(zip(cfg[i]['counts'], cfg[i]['colors'])):
                    bar = ax.bar(pred_x[i], count, bar_width,
                                 bottom=bottom, color=color, alpha=1)
                    split_bars.append((bar, bottom, count, cfg[i]['sizes'][j]))
                    bottom += count
                all_split_bars[i] = split_bars
            else:
                bar = ax.bar(pred_x[i], pred_data[i], bar_width,
                             color=colors[offset_key], alpha=1)
                if pred_sizes[i] is not None:
                    bar_obj = bar[0]
                    ax.text(bar_obj.get_x() + bar_obj.get_width() / 2,
                            bar_obj.get_height() + 500,
                            f'{pred_sizes[i]}B', ha='center', va='bottom',
                            fontsize=11, color=colors[offset_key], fontweight='bold')

        for i, split_bars in all_split_bars.items():
            for j, (bar_container, bot, count, size) in enumerate(split_bars):
                ...
                ax.text(bar.get_x() + bar.get_width() / 2,
                        ...
                        color='black' if j < 3 else 'white', fontweight='bold')
