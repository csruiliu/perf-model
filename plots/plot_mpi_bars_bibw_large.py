...
node0_send_prediction  = [0,  0, 100924]
node0_recv_prediction  = [0,  0, 100580]

# Node 1
node1_send_measurement = [100000, 100000, 100000]
node1_recv_measurement = [100000, 100000, 100000]
node1_send_prediction  = [0,  0, 100540]
node1_recv_prediction  = [0,  0, 100872]

measurement_size = [64, 256, 1024]

node0_send_prediction_size = [None, None, 1024]
node0_recv_prediction_size = [None, None, 1024]
node1_send_prediction_size = [None, None, 1024]
node1_recv_prediction_size = [None, None, 1024]
...
split_configs = {
    0: {  # Node 0
        'send': {
            0: {'counts': [8013]*11, 'sizes': [12,14,16,20,24,28,32,40,48,56,64],
                'colors': ['#F3E5F5','#E1BEE7','#CE93D8','#BA68C8','#AB47BC',
                           '#9C27B0','#8E24AA','#7B1FA2','#6A1B9A','#4A148C','#2D0060']},
            1: {'counts': [20012]*5, 'sizes': [224,256,320,384,448],
                'colors': ['#FFD580','#FFA500','#FF8C00','#FF6600','#CC4400']},
        },
        'recv': {
            0: {'counts': [8059]*11, 'sizes': [12,14,16,20,24,28,32,40,48,56,64],
                'colors': ['#F3E5F5','#E1BEE7','#CE93D8','#BA68C8','#AB47BC',
                           '#9C27B0','#8E24AA','#7B1FA2','#6A1B9A','#4A148C','#2D0060']},
            1: {'counts': [19948]*5, 'sizes': [224,256,320,384,448],
                'colors': ['#FFD580','#FFA500','#FF8C00','#FF6600','#CC4400']},
        },
    },
    1: {  # Node 1
        'send': {
            0: {'counts': [8088]*11, 'sizes': [12,14,16,20,24,28,32,40,48,56,64],
                'colors': ['#F3E5F5','#E1BEE7','#CE93D8','#BA68C8','#AB47BC',
                           '#9C27B0','#8E24AA','#7B1FA2','#6A1B9A','#4A148C','#2D0060']},
            1: {'counts': [19954]*5, 'sizes': [224,256,320,384,448],
                'colors': ['#FFD580','#FFA500','#FF8C00','#FF6600','#CC4400']},
        },
        'recv': {
            0: {'counts': [7991]*11, 'sizes': [12,14,16,20,24,28,32,40,48,56,64],
                'colors': ['#F3E5F5','#E1BEE7','#CE93D8','#BA68C8','#AB47BC',
                           '#9C27B0','#8E24AA','#7B1FA2','#6A1B9A','#4A148C','#2D0060']},
            1: {'counts': [20012]*5, 'sizes': [224,256,320,384,448],
                'colors': ['#FFD580','#FFA500','#FF8C00','#FF6600','#CC4400']},
        ...
        for i in range(len(x_labels)):
            if i in cfg:
                ...
                bar = ax.bar(pred_x[i], pred_data[i], bar_width,
                             color=colors[offset_key], alpha=0.85)
                first_nonsplit = False
