...
export IPM_LOGDIR=/pscratch/sd/r/ruiliu/ipm-logs/milc
...
start=$(date +%s.%N)
srun -N 2 -n $SLURM_NTASKS -c $SLURM_CPUS_PER_TASK --gpus-per-node=1 --cpu-bind=cores ./wrap_dcgmi_container.sh $exe $input
end=$(date +%s.%N)
elapsed=$(printf "%s - %s\n" $end $start | bc -l)

printf "Elapsed Time: %.2f seconds\n" $elapsed > ${MILC_RESULTS}/runtime.out
