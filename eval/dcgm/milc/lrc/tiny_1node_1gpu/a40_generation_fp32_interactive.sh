...
N10_MILC="/global/scratch/users/rliu5/milc-lrc-a40-fp32"
...
qudatune_dir="$PWD/qudatune-generation-a40-fp32"
export QUDA_RESOURCE_PATH=${qudatune_dir}
if [ ! -d ${qudatune_dir} ]; then
    mkdir ${qudatune_dir}
fi

export RESULTS_DIR="${MILC_LRC}/results/MILC_TINY_FP32_${SLURM_JOB_ID}"
mkdir -p ${RESULTS_DIR}
cd ${RESULTS_DIR}
