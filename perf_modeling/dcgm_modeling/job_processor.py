...
class JobProcessor:
    ...
    def _organize_by_file_content(self, all_files: list[Path]) -> list[str]:
        ...
        if len(file_info) != self.num_gpu:
            ...
            raise ValueError(
                f"Expected {self.num_gpu} files but found {len(file_info)}"
            )
        ...
        for logical_gpu_id, (file_path, detected_gpu_id, line_count) in enumerate(
            file_info
        ):
    ...
    def process_files(self, dcgm_input: str) -> list[pd.DataFrame]:
        ...
        if os.path.isdir(dcgm_input):
            ...
            raise ValueError(
                f"Input path '{dcgm_input}' is neither a valid file nor a directory"
            )
    ...
    def _process_multiple_files(self, file_paths: list[str]) -> list[pd.DataFrame]:
        ...
        for logical_gpu_id, file_path in enumerate(file_paths):
            ...
            gpu_data = self._extract_single_gpu_data(
                lines, metric_indices, len(header_columns)
            )
            ...
            if gpu_data:
                ...
                print(
                    f"Logical GPU {logical_gpu_id}: Created DataFrame with {len(gpu_data)} rows"
                )
            else:
        ...
        for line in lines:
            ...
            if self.GPU_PATTERN.match(line):
                ...
                if len(numeric_values) >= num_columns - 1:
                    ...
                    print(
                        f"Warning: Line has insufficient data columns: {line.strip()}"
                    )
