...
class ReferenceProfiler(BaseProfiler):
    ...
    def run(
        ...
        for gpu_id, df in enumerate(gpu_dfs):
            ...
            gpu_metrics = self._process_single_gpu(
                df, metrics, overall_runtime_ms, start_ts, end_ts, tensor_prec
            )
        ...
        self.formatter.print_reference_results(
            aggregated_metrics, flops, mem_bw, self.gpu.get_name()
        )
    ...
    def _slice_and_aggregate(
        self, components_list: list[TimeComponents], time_slice: TimeSlice
    ) -> dict[str, list[float]]:
        """Slice components and add total time"""
        sliced = {
            key: [comp.to_dict()[key] for comp in components_list][
                time_slice.start_idx : time_slice.end_idx
            ]
        ...
        sliced["t_total"] = [
            ...
            + sliced["t_pcie"][i]
            + sliced["t_nvlink"][i]
            + sliced["t_othernode"][i]
            for i in range(len(sliced["t_kernel"]))
        ]

        return sliced

    def _aggregate_multi_gpu(
        self, all_gpu_metrics: list[dict[str, list[float]]], agg_interval_ms: float
    ) -> dict[str, list[float]]:
...
class TargetPredictor(BaseProfiler):
    ...
    def run(
        ...
        for gpu_id, df in enumerate(gpu_dfs):
            ...
            gpu_metrics = self._predict_single_gpu(
                df, metrics, overall_runtime_ms, start_ts, end_ts, tensor_prec
            )
            all_gpu_metrics.append(gpu_metrics)
        ...
        # Print predictions
        self.formatter.print_target_results(
            aggregated_metrics, est_flops, est_mem_bw, self.tgt_gpu.get_name()
        )
    ...
    def _calc_target_metrics(
        self, df: pd.DataFrame, metrics: list[str], tensor_prec: str
    ) -> dict[str, list[float]]:
        ...
        for row in df.itertuples(index=False):
            ...
            # Calculate totals
            results["t_total_lower"].append(
                results["t_kernel_lower"][-1] + t_pcie + t_nvlink + t_othernode
            )
            results["t_total_mid"].append(
                results["t_kernel_mid"][-1] + t_pcie + t_nvlink + t_othernode
            )
            results["t_total_upper"].append(
                results["t_kernel_upper"][-1] + t_pcie + t_nvlink + t_othernode
            )
