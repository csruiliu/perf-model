...
class TargetPredictor(BaseProfiler):
    ...
    def _calc_target_metrics(
        self, profiled_df: pd.DataFrame, metrics: list[str], cores_alloc: str
    ) -> dict[str, list[float]]:
    ...
    def _calculate_all_scales(
        self, scale_calc: GPUScaleCalculator, intensities: dict, tf_weights: dict
    ) -> dict[str, tuple]:
    ...
    def _calc_aggregated_metrics(
        self, results: dict[str, list], src_prefix: str, tgt_prefix: str
    ) -> dict[str, float]:
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        required=True,
        choices=["same", "all"],
        help="CPU Cores Allocation Strategy (optional)",
    )
    parser.add_argument(
        "--metrics",
        type=lambda s: s.split(","),
        required=True,
        help="Comma-separated list of metrics (e.g., GRACT, DRAMA, SMOCC)",
    )
