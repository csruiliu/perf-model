...
if [ "${SLURM_LOCALID}" -eq 0 ]; then
    ...
    if [ -d "${TELEM_PATH}" ]; then
        ...
        # -----------------------------------------------------------------
        # FIX #4: Register a trap to kill the background monitor process
        # on EXIT (normal exit, errors), INT (Ctrl-C), and TERM (SLURM
        # scancel / timeout). This ensures the monitor is never left
        # running as a zombie if the benchmark crashes or is killed.
        # The trap checks whether telemetry_pid is set and the process
        # still exists before attempting to kill it, so it is safe to
        # call even after the explicit kill/wait block below has already
        # cleaned up on a normal exit.
        # -----------------------------------------------------------------
        trap '
            if [ -n "${telemetry_pid}" ] && kill -0 "${telemetry_pid}" 2>/dev/null; then
                kill -9 "${telemetry_pid}" 2>/dev/null
                wait "${telemetry_pid}" 2>/dev/null
                echo "Monitor force-stopped (trap) on ${NODE}"
            fi
        ' EXIT INT TERM
...
# =============================================================
# Stop monitoring after benchmark completes (clean-exit path)
# =============================================================
if [ "${SLURM_LOCALID}" -eq 0 ]; then
    if [ -n "${telemetry_pid}" ] && kill -0 "${telemetry_pid}" 2>/dev/null; then
        kill -9 "${telemetry_pid}"
        wait "${telemetry_pid}" 2>/dev/null
        # Disarm the trap now that we have already cleaned up, so the
        # EXIT trap does not print a redundant "force-stopped" message.
        trap - EXIT INT TERM
